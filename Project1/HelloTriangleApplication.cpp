...
void HelloTriangleApplication::createSwapChain() {
    ...
    //图形队列族与呈现队列族不同，使用并发模式，图像可以在多个队列族之间使用，无需显式的所有权转移
...
void HelloTriangleApplication::createGraphicsPipeline() {
    ...
    //固定功能状态：定义管线固定功能阶段的所有结构，比如输入汇编、光栅化、视口与颜色混合
    //管线布局：着色器引用、可在绘制时更新的uniform和push值
    //渲染通道：管线阶段引用的附件及其用法
...
void HelloTriangleApplication::createUniformBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        //映射缓冲区
        //持久映射，不必每次需要更新缓冲区时都映射缓冲区，可以提高性能
...
void HelloTriangleApplication::createTextureImage() {
    ...
    memcpy(data, piexls, static_cast<size_t>(imageSize));
    vkUnmapMemory(device, stagingBufferMemory);
...
void HelloTriangleApplication::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    ...
    //未定义->传输目标：不需要等待任何的传输写入
    //传输目标->着色器读取：着色器读取应等待传输写入，特别是片段着色器中的着色器读取
